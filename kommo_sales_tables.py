...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  #t-source th:not(.c-metric),#t-source td:not(.c-metric){min-width:112px}
  #t-source .c-metric{min-width:210px}
...
<div class="box">
  <h2>Sumber Leads — deal yang Closed - Won</h2>
  <div class="sub">Semua tipe customer digabung (B2C + B2B + tanpa Customer Type).
    Dihitung dari tanggal closing, sama seperti baris Sales di atas</div>
...
// ── Sumber Leads for won deals, all customer types ────────────────────────────
function renderSource(ym, cols){
  const dates = new Set(cols.flatMap(c => c.dates));
  const agg = {};
  let nWon = 0, vWon = 0, nSch = 0, vSch = 0;

  LEADS.forEach(l => {
    if(!l.ad || !dates.has(l.ad)) return;          // same date basis as the Sales rows
    const a = agg[l.sl] || (agg[l.sl] = {won:0, wonRp:0, sch:0, schRp:0});
    if(l.as === 'won'){ a.won++; a.wonRp += l.p; nWon++; vWon += l.p; }
    else              { a.sch++; a.schRp += l.p; nSch++; vSch += l.p; }
  });

  const rows = Object.entries(agg).sort((a,b) =>
    (b[1].won - a[1].won) || (b[1].wonRp - a[1].wonRp));

  let h = '<thead><tr>' +
    '<th class="c-metric">Sumber Leads</th>' +
    '<th>Closed - Won</th><th>Closed - Won — Rp</th><th>% dari Rp</th>' +
    '<th>Work Scheduled</th><th>Work Scheduled — Rp</th>' +
    '<th class="c-total">Total Rp</th></tr></thead><tbody>';

  if(!rows.length){
    h += `<tr><td class="c-metric">—</td><td colspan="6" style="text-align:left;color:#b2bec3">` +
         `Belum ada deal yang closing di bulan ini</td></tr>`;
  }
  rows.forEach(([src, a]) => {
    const tot = a.wonRp + a.schRp;
    const pct = vWon ? (a.wonRp / vWon * 100) : 0;
    h += `<tr><td class="c-metric">${src}</td>` +
         `<td>${fmtInt(a.won)}</td>` +
         `<td title="${fmtRpFull(a.wonRp)}">${fmtRp(a.wonRp)}</td>` +
         `<td>${a.wonRp ? pct.toFixed(0)+'%' : '<span class="zero">—</span>'}</td>` +
         `<td>${fmtInt(a.sch)}</td>` +
         `<td title="${fmtRpFull(a.schRp)}">${fmtRp(a.schRp)}</td>` +
         `<td class="c-total" title="${fmtRpFull(tot)}">${fmtRp(tot)}</td></tr>`;
  });
  if(rows.length){
    h += '<tr class="r-total"><td class="c-metric">TOTAL</td>' +
         `<td>${fmtInt(nWon)}</td><td title="${fmtRpFull(vWon)}">${fmtRp(vWon)}</td>` +
         `<td>${vWon ? '100%' : '<span class="zero">—</span>'}</td>` +
         `<td>${fmtInt(nSch)}</td><td title="${fmtRpFull(vSch)}">${fmtRp(vSch)}</td>` +
         `<td class="c-total" title="${fmtRpFull(vWon+vSch)}">${fmtRp(vWon+vSch)}</td></tr>`;
  }
  document.getElementById('t-source').innerHTML = h + '</tbody>';
}
